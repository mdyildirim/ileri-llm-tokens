...
import { AppState, AppSettings, ProviderSettingsData, ProviderSetting } from '../types';
...
export const RunControls: React.FC<RunControlsProps> = ({ settings, onSettingsChange, onRun, onCancel, onReset, appState, datasetSize, providerSettings }) => {
    ...
    return (
        <div className="p-6 bg-white dark:bg-bunker-900/70 rounded-2xl shadow-lg backdrop-blur-sm border border-white/20">
            <h2 className="text-2xl font-bold mb-4 text-bunker-800 dark:text-bunker-100">3. Run Benchmark</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label htmlFor="concurrency" className="block text-sm font-medium text-bunker-700 dark:text-bunker-300">Concurrency</label>
                    ...
                    <span className="text-sm text-bunker-500 dark:text-bunker-400">{settings.concurrency} parallel requests</span>
                </div>
                <div>
                    <label htmlFor="delay" className="block text-sm font-medium text-bunker-700 dark:text-bunker-300">Delay</label>
                    ...
                    <span className="text-sm text-bunker-500 dark:text-bunker-400">{settings.delay} ms between requests</span>
                </div>
                <div className="sm:col-span-2">
                    <label htmlFor="runFirstN" className="block text-sm font-medium text-bunker-700 dark:text-bunker-300">Process Rows</label>
                    <input type="number" value={settings.runFirstN} onChange={(e) => onSettingsChange({ runFirstN: parseInt(e.target.value) || 0 })} disabled={isRunning} id="runFirstN" placeholder={`All ${datasetSize} rows`} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-bunker-300 dark:border-bunker-600 bg-white dark:bg-bunker-800 dark:text-bunker-200 focus:outline-none focus:ring-sky-500 focus:border-sky-500 sm:text-sm rounded-md" />
                </div>
                <div className="sm:col-span-2 flex items-center">
                    ...
                    <label htmlFor="regenerate-tr-nodia" className="ml-2 block text-sm text-bunker-600 dark:text-bunker-400">Re-generate 'tr_nodia' column client-side</label>
                </div>
            </div>
            <div className="mt-6 p-3 bg-sky-100 dark:bg-sky-500/10 rounded-lg text-center text-sm">
                <p className="text-sky-800 dark:text-sky-200"><strong>Cost Note:</strong> A rough estimate to process the selected rows might be <strong>{estimatedCost()}</strong>. This is not a guarantee.</p>
            </div>
